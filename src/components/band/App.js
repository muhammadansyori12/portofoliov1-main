...
function Band({ isMobile, maxSpeed = 50, minSpeed = 10 }) {
  ...
  const profileTexture = useTexture('/assets/PP.png');
  
  // Fix texture centering and flip
  profileTexture.flipY = false;
  profileTexture.center.set(0.5, 0.5);
  
  // Tambahkan ini untuk mengatur ukuran dan posisi foto agar center
  profileTexture.repeat.set(1.2, 1.2); // Ubah angka ini untuk efek zoom (misal 1.5 atau 0.8)
  profileTexture.offset.set(0.3, 0.);     // Ubah nilai X (kiri) dan Y (kanan) untuk menggeser foto
  
  // Jika foto terlihat miring/menyamping, ubah nilai 0 di bawah menjadi Math.PI / 2 atau -Math.PI / 2
  profileTexture.rotation = 0; 
  
  // Mencegah warna foto menjadi pudar (menyesuaikan sRGB color space standar Three.js)
          ...
          >
            <mesh geometry={nodes.card.geometry}>
              <meshPhysicalMaterial 
                {...materials.base} 
                map={profileTexture}
            // Menggunakan warna putih agar tekstur foto asli tidak tercampur/menjadi gelap
            color="#ffffff" 
              />
            </mesh>
